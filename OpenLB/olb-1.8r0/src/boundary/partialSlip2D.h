 ...
 *
 *  Copyright (C) 2020 Fedor Bukreev, Alexander Schulz
...
#ifndef PARTIAL_SLIP_2D_H
#define PARTIAL_SLIP_2D_H

#include "partialSlip.h"
...
std::optional<PostProcessorPromise<T,DESCRIPTOR>> getPostProcessor(DiscreteNormalType type,
                                                                   DiscreteNormal<DESCRIPTOR> n) {
  if (n != 0) {
    return boundaryhelper::promisePostProcessorForNormal<T,DESCRIPTOR,PartialSlipO2D>(n);
  }
  return std::nullopt;
}
