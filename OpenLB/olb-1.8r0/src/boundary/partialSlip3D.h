...
#ifndef PARTIAL_SLIP_3D_H
#define PARTIAL_SLIP_3D_H
...
template <int NX, int NY, int NZ> struct PartialSlipO3D;
...
std::optional<PostProcessorPromise<T,DESCRIPTOR>> getPostProcessor(DiscreteNormalType type,
                                                                   DiscreteNormal<DESCRIPTOR> n) {
  if (n != 0) {
    return boundaryhelper::promisePostProcessorForNormal<T,DESCRIPTOR,PartialSlipO3D>(n);
  }
  return std::nullopt;
}
