...
def cse(block, generator):
    ...
    #return block.cse(symbols=generator, order='none')
...
def raw_collision(collision, descriptor, momenta, equilibrium, concrete = None):
    if concrete is None:
    ...
    return '\n'.join([
        f"""template <concepts::Cell CELL, typename PARAMETERS, typename V=typename CELL::value_t>""",
...
def compute_method_cse(struct, method, **args):
    print(f"Generating { struct.__cpp_name__ }::{ method }")
    # Distinct symbols for all extracted subexpressions and helper variables
    generator = iter(SymbolGenerator('x'))
    # Generate symbols for arguments
    ...
    for name, arg in args.items():
        ...
        symbols = symbols | arg.symbols
    # Apply operator to arguments
    result = getattr(struct(), method)(*[ arg.expr for arg in args.values() ])
    # Import returned expression tree from C++ into Python context
    if result != None:
        result = import_expr(result, symbols)
    # Import expression trees of modified arguments from C++ into Python context
    ...
    # Generate C++ template for CSE-ified operator
    return '\n'.join([
        f"""template <{ ', '.join([ "typename " + '_' + arg.name.upper() for arg in args.values() ]) }, typename V=typename _{ list(args.values())[0].name.upper() }::value_t>""",
        f"""{ result_type } { method }({ ', '.join([ "const _" + arg.signature() for arg in args.values() ]) }) any_platform""",
        "{",
        code(block, symbols=generator.symbols),
        optional,
        "}"
...
def dual_dynamics_cse(dynamics, simple_post_collision = True):
    descriptor = dynamics.descriptor_t
    concrete = olb.collision.Dual[dynamics.CollisionO]
    print(f"Generating { concrete.__cpp_name__ }")
    # Distinct symbols for all extracted subexpressions and helper variables
    generator = iter(SymbolGenerator('x'))
    # Setup symbolic cell and parameter for collision
    cell = symbolic.Cell(descriptor, 'cell', generator)
    parameters = symbolic.Parameters(descriptor, dynamics.parameters, 'parameters', generator)
    # Collect symbols used by cell and parameters as evaluation context
    symbols = cell.symbols | parameters.symbols
    # Apply collision operator
    result = concrete().apply(cell.expr, parameters.expr)
    ...
    return '\n'.join([
        "template <>",
        f"struct Dual<{ dynamics.CollisionO.__cpp_name__ }>",
        "{",
