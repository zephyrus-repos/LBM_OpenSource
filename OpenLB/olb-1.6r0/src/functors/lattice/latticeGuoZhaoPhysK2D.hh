  ...
  : SuperLatticePhysF2D<T, DESCRIPTOR>(sLattice, converter, 1)
{
  ...
  for (int iC = 0; iC < maxC; iC++) {
    this->_blockF.emplace_back(new BlockLatticeGuoZhaoPhysK2D<T,DESCRIPTOR>(this->_sLattice.getBlock(iC), this->_converter));
  }
}
...
{
  output[0] = this->_blockLattice.get(input[0], input[1]).template getFieldComponent<descriptors::K>()
