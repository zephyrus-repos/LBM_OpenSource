...
void setRtlbmDiffuseConstTemperatureBoundary(SuperLattice<T, DESCRIPTOR>& sLattice, T omega, FunctorPtr<SuperIndicatorF3D<T>>&& indicator)
{
  ...
  for (int iCloc = 0; iCloc < sLattice.getLoadBalancer().size(); iCloc++) {
    setRtlbmDiffuseConstTemperatureBoundary<T,DESCRIPTOR>(sLattice.getBlock(iCloc),
    ...
    if (blockGeometryStructure.getNeighborhoodRadius({iX, iY, iZ}) >= margin
      ...
      if (discreteNormal[0] == 0) { // flat //set momenta, dynamics and postProcessors for indicated cells
        if (discreteNormal[1] != 0 && discreteNormal[1] == -1) {
          dynamics = _block.template getDynamics<RtlbmDiffuseConstBoundaryDynamics<T,DESCRIPTOR,
