...
#ifndef GPU_HIP_COLUMN_H
#define GPU_HIP_COLUMN_H
...
template <typename T>
struct ImplementationOf<AbstractColumn<T>,Platform::GPU_HIP> {
  using type = gpu::hip::Column<T>;
};

/// Declare gpu::cuda::CyclicColumn as the AbstractCyclicColumn implementation for GPU CUDA targets
template <typename T>
struct ImplementationOf<AbstractCyclicColumn<T>,Platform::GPU_HIP> {
  using type = gpu::hip::CyclicColumn<T>;
};
 ...
 **/
template <typename T>
class ConcreteCommunicatable<gpu::hip::Column<T>> final : public Communicatable {
private:
  gpu::hip::Column<T>& _column;

public:
  ConcreteCommunicatable(gpu::hip::Column<T>& column):
    _column{column} { }
...
template <typename T>
class ConcreteCommunicatable<gpu::hip::CyclicColumn<T>> final : public Communicatable {
private:
  gpu::hip::CyclicColumn<T>& _column;

public:
  ConcreteCommunicatable(gpu::hip::CyclicColumn<T>& column):
    _column{column} { }
