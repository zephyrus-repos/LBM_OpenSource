...
#ifndef GPU_HIP_MASK_H_
#define GPU_HIP_MASK_H_
...
template <typename T>
class ConcreteBlockMask<T,Platform::GPU_HIP> final : public Serializable {
private:
  gpu::hip::Column<bool> _mask;
  std::size_t _weight;
...
template<typename T>
std::size_t ConcreteBlockMask<T,Platform::GPU_HIP>::getNblock() const
{
  return 1 + _mask.getNblock();
}

template<typename T>
std::size_t ConcreteBlockMask<T,Platform::GPU_HIP>::getSerializableSize() const
{
  return sizeof(_weight) + _mask.getSerializableSize();
}

template<typename T>
bool* ConcreteBlockMask<T,Platform::GPU_HIP>::getBlock(std::size_t iBlock, std::size_t& sizeBlock, bool loadingMode)
{
